...
status: 分析完成；§7 的收斂調整已於 commit 1b83eb4 執行
created: 2026-09-04
updated: 2026-09-04
...
| **B. 指標檔（採用）** | `AGENTS.md` 縮成數行指向 `CLAUDE.md`，本身不放規則 | ✅ 立即可做 | 零工具依賴、跨平台、單一真實來源、與 `README.md:80` 既有做法一致 | 依賴讀 `AGENTS.md` 的 agent 願意跟著開 `CLAUDE.md`（故指標必須寫成「必讀」而非普通連結） |
...
## 7. 已執行的調整（commit `1b83eb4`）

依 §5 方案 B：

- `AGENTS.md` 獨有且工具無關的內容（安裝／測試指令、程式碼風格、測試規範、commit/PR 規則）**併入 `CLAUDE.md`**，語言統一為中文。
- `AGENTS.md` 縮成純指標檔，**本身不再保留任何規則**，並在檔內註明為何不重複（連結本報告）。
- `AGENTS.md` 納入版控（決定 2）。
- `CLAUDE.md` 的「目錄慣例」加一行說明這個指標安排。

**尚未執行**：決定 3（README 重複）、決定 4（抽出 `docs/project-status.md`）、決定 5（#8 是否升為共通規範）——這三項需要你拍板後才動。
